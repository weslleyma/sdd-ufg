...
public class GradeResource extends AbstractResource {
    ...
    public Response retrieveAllGrades(@QueryParam("page") Integer page, @Context final HttpServletRequest request) {
		...
		List<Grade> grades = gradeDao.findAll(0);
		if (page == null) {
			page = 1;
		}
		...
		ResultSetResponse<Grade> rsp = new ResultSetResponse<Grade>(grades, page);
	...
	private Grade retrieveGradeFromJson(final HttpServletRequest request) throws Exception {
		Map<String, Object> content = getJSONContent(request);
		
		Grade grade = new Grade();
		grade.setName(content.get("name").toString());
		
		Course course = courseDao.findById(new Long(content.get("course_id").toString()), 0);
